...
const divisibleByFiveTwoToThePower = (input) => {
  const newArr=input.map(element => {
    return element.filter(value => {
      if (typeof (value) === 'number' && value % 5 === 0) return value;
...
let findMaleAndFemale = (data) => {
  ...
  for (const item in data){
      ...
      if (data[item].gender == 'female' ||               
        ...
        newStr.push(data[item].name);
      };
  
  };
   
  
    return newStr.join(' and ');
...
map and reduce to return the name of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {

  return data.reduce((accumelator,element) => {
    if(parseInt(element.height) < accumelator['hight']) {
      accumelator['hight']=parseInt(element.height);
      accumelator['name'] =element.name;
    }
    return accumelator;
  },{hight:700,name:''}).name;
};
